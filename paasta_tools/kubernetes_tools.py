        ...
        for k, v in self.get_env().items():
            if is_secret_ref(v):
                if is_shared_secret(v):
                    ...
                    secret_env_vars.append[k] = v

        user_env = [
            V1EnvVar(name=name, value=value) for name, value in self.get_env().items()
            if name not in secret_env_vars.keys() + shared_secret_env_vars.keys()
        ]
        user_env.append(self.get_kubernetes_secret_env_vars(
            ...
            ret.append(V1EnvVar(
                ...
                value_from=V1EnvVarSource(
                    secret_key_ref=V1SecretKeySelector(
                        key=f'paasta-secret-{service}-{secret}',
                        optional=False,
                    ),
                ),
            ))
        for k, v in shared_secret_env_vars:
